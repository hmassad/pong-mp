...
class GameState:
    ...
    def handle_player1(self, dt):
        if player1_computer:
            ...
            if diff < PADDLE_SPEED_CPU * dt:
                self.player1.y += diff
            else:
                self.player1.y += PADDLE_SPEED_CPU * dt
    ...
    def handle_player2(self, dt):
        if player2_computer:
            ...
            if diff < PADDLE_SPEED_CPU * dt:
                ...
                self.player2.y += PADDLE_SPEED_CPU * dt
        else:
            if keymap[pyglet.window.key.UP]:
    ...
    def update(self, dt):
        ...
        elif self.ball.x < PADDLE_WIDTH and self.ball.y <= self.player1.y + PADDLE_HEIGHT / 2 and self.ball.y >= self.player1.y - PADDLE_HEIGHT / 2:
